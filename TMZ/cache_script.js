...
! function a(b, c, e) {
		...
		! function(e, h, p, n, k, g, q, j, l, m, i, b, c, d, f, o) {
			...
			e = function(a, c) {
				...
				if (a.currentStyle ? b = a.currentStyle[c] : window.getComputedStyle && (b = document.defaultView.getComputedStyle(a, null).getPropertyValue(c)), b == 'auto' && c == 'cursor') {
					var e = ['a'];
					for (var d = 0; d < e.length; d++) if (a.tagName.toLowerCase() == e[d]) return 'pointer'
				}
				return b
			}, h = function(a) {
	...
	2: [function(h, j, i) {
		var c = typeof self !== 'undefined' ? self : typeof window !== 'undefined' ? window : {}, e = c.ZeroClipboard = h('ZeroClipboard'),
			g = {
...
define('omniture', ['window', 'logger'], function(window, logger) {
	return window.s || {
		t: function t() {
			logger.getInstance().error('Omniture is not available, t() will not be tracked.', arguments);
		},
		tl: function tl() {
			logger.getInstance().error('Omniture is not available, tl() will not be tracked.', arguments);
		}
	};
});
...
this["JST"]["users/nav-buttons"] = Handlebars.template(function(Handlebars, depth0, helpers, partials, data) {
	...
	function program1(depth0, data) {
		var buffer = "",
			stack1;
		buffer += "\n                <li><a href=\"";
		if (stack1 = helpers.UPLOAD_SITE_BASEURL) {
			stack1 = stack1.call(depth0, {
				hash: {},
				data: data
			});
		} else {
			stack1 = depth0.UPLOAD_SITE_BASEURL;
	...
	if (stack1 = helpers.display_name) {
		stack1 = stack1.call(depth0, {
			hash: {},
			data: data
		});
	...
	stack1 = helpers['if'].call(depth0, depth0.is_logged_in, {
		hash: {},
...
define('app', ['require', 'jquery', 'dispatcher', 'util', 'logger', 'module', 'window', 'document'], function(require, $, dispatcher, util, loggerFactory, module, window, document, undefined) {
	...
	function app() {
		...
		function init() {
			$('a:not([href^="' + window.SITE_BASEURL + '"]):not([href^="http://photos.toofab.com"]):not([href^="#"]):not([href^="/"])').attr('target', '_blank').addClass('external');
			logger.info('init');
		}
		_this.init = init;
...
define('loaders/kaltura-kwidget', ['require', 'jquery', 'dispatcher', 'wbppid', 'wbkrux', 'logger', 'module', 'window', 'document'], function(require, $, dispatcher, wbppid, wbkrux, loggerFactory, module, window, undefined) {
	...
	function loader() {
		...
		function load(partnerId, uiconfId) {
			if (requested) {
				logger.info('load :: already requested');
				return scriptReadyPromise;
			}
			if (!partnerId || !uiconfId) {
				logger.warn('load :: cannot request kWidget without partnerId and uiconfId');
				return scriptReadyPromise;
			}
			requested = true;
			...
			require([scriptUrl], function() {
				if (!window.kWidget) {
					logger.error('load :: window.kWidget never got defined', window.kWidget);
					scriptReadyDeferred.reject();
				} else {
					if (window.mw) {
						configureMwEmbed(window.mw);
					}
		...
		function configureMwEmbed(mw) {
			mw.setConfig('Kaltura.EnableEmbedUiConfJs', true);
			dispatcher.trigger('kaltura:player:global-configure-mwembed', mw);
		}

		function configureKWidget(kWidget) {
			dispatcher.trigger('kaltura:player:global-configure-kwidget', kWidget);
		}
		...
		function configureKWidgetEmbed(embed) {
			...
			if ('tpxnoads' != adzone && 'prod' != window.SYSTEM_ENV) {
				adzone += '-dev';
			}
			var defaults = {
				entry_id: entryId,
	...
	function analytics() {
		...
		function bindTrackLinkHandlers() {
			$(document).on('click', '.js-track-link', function(e) {
				...
				if (!context) {
					logger.error('onTrackLink :: "context" cannot be empty in data-event payload.', [$link[0], linkEvent]);
					return true;
				}
				if (!href) {
					logger.error('onTrackLink :: "href" must be set on link element.', [$link[0], linkEvent]);
					return true;
				}
				var opensInSameWindow = !target || '_self' == target || '_top' == target || '_parent' == target || window.name && window.name == target;
				if (window.BASE_TARGET_BLANK) {
					opensInSameWindow = false;
				}
				...
				var eventData = {
					action: action,
					label: title || $link[0].textContent || href || 'unknown'
				};
				try {
					trackEvent(eventName, eventData, null, function handleTrackLinkCallback() {
						if (opensInSameWindow) {
							window.location.href = href;
						} else {
							logger.info('onTrackLink :: success', [eventName, eventData]);
						}
					});
				} catch (err) {
					logger.error('onTrackLink :: error', [err, $link[0], linkEvent, eventName, eventData]);
					if (opensInSameWindow) {
						window.location.href = href;
					}
				}
		...
		function trackPageView(page) {
			...
			s.events = 'event6';
			if (page.initial_load && page.search_query) {
				s.events += ',event19';
			}
			dispatcher.trigger('page:track_view.omniture.before', {
				s: s,
				page: page
			});
			s.t();
			if (s.campaign) {
				ga('set', 'campaignId', s.campaign);
			}
			ga('set', 'title', title);
			ga('send', 'pageview');
		...
		function trackEvent(event, data, options, callback) {
			...
			$.when(gaDeferred, sDeferred).always(function() {
				clearTimeout(t);
				if ($.isFunction(callback)) {
					callback();
				}
			...
			ga('send', {
				...
				eventAction: action,
				eventLabel: label,
				hitCallback: function resolveGa() {
					gaDeferred.resolve();
				}
			});
		}
...
define('toofab/griddler/1.0.0/griddler', ['jquery', 'analytics', 'dispatcher', 'page', 'logger', 'module', 'window', 'document'], function($, analytics, dispatcher, page, loggerFactory, module, window, document, undefined) {
	...
	function griddler(element, options) {
		...
		function init() {
			...
			if (!settings.loadMoreUrl) {
				logger.info('init :: loadMoreUrl was blank, must be no more items available.');
				return _this;
			}
			...
			dispatcher.trigger('griddler:init:complete', {
				element: $elm
			});
			return _this;
		}
		...
		function updateControls() {
			logger.debug('updateControls', settings);
		...
		function loadMoreContent() {
			if (!settings.loadMoreUrl) {
				logger.warn('loadMoreContent :: loadMoreUrl is blank, abort!');
				updateControls();
				return;
			}
			analytics.trackEvent(settings.container + ':' + settings.title + ':griddler:load-more:' + curPage, {
				action: 'load-more',
				label: 'collection:' + settings.title + '-page-' + curPage
			});
			$.ajax({
				url: settings.loadMoreUrl,
				html: true
				...
				dispatcher.trigger('griddler:load_more:success', {
					element: $elm,
					page: curPage,
					url: settings.loadMoreUrl
				});
				...
				setLoadMoreUrl('');
				dispatcher.trigger('griddler:load_more:fail', {
					element: $elm,
					page: curPage,
					url: settings.loadMoreUrl
				});
		...
		function setLoadMoreUrl(url) {
			settings.loadMoreUrl = $.trim(url);
			updateControls();
			return _this;
		}
		_this.init = init;
...
define('toofab/navtabs/1.0.0/navtabs', ['jquery', 'window', 'document'], function($, window, document, undefined) {
	...
	var moduleDefaults = {
		debug: false,
		container: '',
		ajaxSlotBaseUrl: '/_/ajax/promotion-slot/',
		allowedDomain: window.location.href
	};
	...
	function module(element, options) {
		...
		function bindControls() {
			...
			$tabs.find('a').on(mouseoverEvent, function(e) {
				e.stopPropagation();
				e.preventDefault();
				tabsOpen = 1;
				var $tab = $(this).parent();
		...
		function loadTabContent(tabIdx, tabSlot) {
			...
			if (settings.container) {
				url += '?Container=' + encodeURIComponent(settings.container);
			}
			$.ajax({
				url: url,
				html: true
				...
				$target.html('Unable to load tab content: ' + msg);
			});
			loadedTabs[tab] = $target;
...
define('toofab/search-box/1.0.0/search-box', ['jquery', 'modernizr', 'dispatcher', 'window', 'document'], function($, Modernizr, dispatcher, window, document, undefined) {
	...
	function searchBox() {
		...
		function getResults($form) {
			...
			var q = evt.$input.val();
			q = $.trim(q);
			if (q == '' || q == defaultText || evt.abort) {
				window.alert(evt.message || 'Please enter a search query.');
				$input.focus();
				return false;
			}
			$form[0].submit();
...
define('toofab/tetris/1.0.0/tetris', ['jquery', 'analytics', 'dispatcher', 'page', 'logger', 'module', 'util', 'window', 'document'], function($, analytics, dispatcher, page, loggerFactory, module, util, window, document, undefined) {
	...
	function tetris(element, options) {
		...
		function init() {
			...
			dispatcher.trigger('tetris:init:complete', {
				element: $elm
			});
			return _this;
		}
		...
		function loadMoreContent() {
			...
			analytics.trackEvent(settings.container + ':' + settings.title + ':tetris:load-more:' + curPage, {
				action: 'load-more',
				label: 'collection:' + settings.title + '-page-' + curPage
			});
			$.ajax({
				...
				page.title_slug = page.title_slug.split('-page-')[0] + '-page-' + curPage;
				analytics.trackPageView(page);
				dispatcher.trigger('tetris:load_more:success', {
					element: $elm,
					page: curPage,
					url: settings.loadMoreUrl
				});
				$content.append(data);
				util.parseSharing($content.get(0));
			}).fail(function(msg) {
				setLoadMoreUrl('');
				dispatcher.trigger('tetris:load_more:fail', {
					element: $elm,
					page: curPage,
					url: settings.loadMoreUrl
				});
...
var wbads = (function($, window, document, undefined) {
		...
		$.when.apply($, deferreds).then(function() {
			...
			$.each(slots, function(index, slot) {
				...
				if (dfp_settings.enable_single_request) {
					if (refreshable) {
						pushCmd(function() {
							googletag.pubads().refresh([adSlotData]);
						});
					...
					if (refreshable) {
						if (!registered) {
							pushCmd(function() {
								googletag.display(adDiv.attr('id'));
	...
	function setRequiredParam(param, val) {
		...
		if (typeof(required_params[param]) == "string") required_params[param] = trim(required_params[param]);
		return true;
	}
	...
	function debug(msg) {
		...
		if (window.console && typeof console.log != "undefined") console.log("[WBADS] " + msg);
	}
	...
	function getQCSegs() {
		if (!settings.quantcast.enabled) return "";
		if (settings.quantcast.segs != "") return settings.quantcast.segs;
		var segs = [];
...
(function(root, factory) {
	if (typeof bootstrap === 'function') {
		bootstrap('WbSodaHead', factory(root.$));
	} else if (typeof exports === "object") {
		module.exports = factory(require('jquery'));
	} else if (typeof define === "function" && define.amd) {
		define('wb/sodahead/1.1.2/sodahead', ['jquery'], factory);
	} else {
		root.WbSodaHead = factory(root.$);
	}
})(this, function($) {
	...
	var module = function() {
		...
		var init = function() {
			$('<iframe id="' + iframeId + '" name="' + iframeId + '" style="display:none;"></iframe>').appendTo('body');
			return _this;
		};
		var processQueue = function() {
			...
			fetchPolls(queuedPolls).then(function() {
				queueDeferred.resolve();
			}, function() {
		...
		var fetchPolls = function(ids, deferred, page) {
			...
			log('fetchPolls::' + url);
			if (page > 4) {
				log('fetchPolls::too much recursion.');
				deferred.resolve();
				return deferred.promise();
			}
			if (pollIds == '') {
				log('fetchPolls:: blank pollIds');
				deferred.resolve();
				return deferred.promise();
			}
			$.ajax({
				...
				if (data.poll) {
					if (polls[data.poll.id]) {
						pollsData[data.poll.id] = data.poll;
				...
				if (data.polls) {
					...
					for (var i = 0; i < pollCount; i++) {
						...
						if (polls[poll.id]) {
							pollsData[poll.id] = poll;
							renderPolls(poll.id);
							hasValidPoll = true;
						} else {
							log('fetchPolls:: (multiple) never requested poll id: ' + data.poll.id);
						}
					}
					var hasNext = data.next || false;
					if (hasNext !== false && data.next.length > pollIds.length) {
						page++;
				...
				log('fetchPolls::failed:' + xhr.status + ' -> ' + xhr.statusText);
				deferred.reject();
			});
			return deferred.promise();
		};
		var renderPolls = function(pollId) {
			$.each(polls[pollId], function(domId, pollObj) {
				if (pollsData[pollId] && pollsData[pollId].answers) {
					pollObj.data = pollsData[pollId];
					renderPoll(pollObj);
				} else {
					log('renderPolls::no answers property on data.');
		...
		var renderPoll = function(poll) {
			...
			for (var i = 0; i < answerCount; i++) {
				...
				if (answer.totalVotes > 0) {
					answerPercent = hasAnyVotes ? Math.round((answer.totalVotes / poll.data.totalVotes) * 100) : 100;
				} else {
					answerPercent = 0;
				}
				answersHtml += '<div class="answer clearfix"><label for="' + answerDomId + '-af">' + escapeHtml(answer.title) + '</label><div><input type="radio" name="answer" id="' + answerDomId + '-af" value="' + answer.id + '"/></div></div>';
				resultsHtml += '<div class="result clearfix"><div class="title">' + escapeHtml(answer.title) + '</div>' + '<div class="percent"><div class="meter" style="width:' + answerPercent + '%;"></div><span>' + answerPercent + '%</span></div></div>';
			}
			if (!poll.data.closed) {
				var serviceURL;
				if ($.browser.msie) {
					serviceURL = '/wb-sodahead/proxy/polls/' + poll.id + '/vote/';
				} else {
					serviceURL = apiURL + 'polls/' + poll.id + '/vote/';
				}
				...
				poll.$voteButton.on('click', function(e) {
					e.stopPropagation();
					e.preventDefault();
					vote(poll);
					return false;
				});
				poll.$answers.on('click', '.answer', function(e) {
					e.stopPropagation();
					e.preventDefault();
				...
				if (poll.data.hideResults) {
					if (poll.data.hideResults_content) {
						resultsHtml = '<p>' + escapeHtml(poll.data.hideResults_content) + '</p>';
					} else {
						resultsHtml = '<p>This poll is now closed.</p>';
					}
				}
				poll.$results.html(resultsHtml);
				poll.$results.show();
		...
		var vote = function(poll) {
			...
			if (parseInt(answer) != answer) {
				poll.$errors.html('<p>Please select an answer.</p>').slideDown('fast');
				return;
			}
		...
		var addPoll = function(pollId, domId) {
			var $elm = $('#' + domId);
			if (!$elm || parseInt(pollId) != pollId) {
				log('addPoll::failed, no dom node with id ' + domId);
				return;
			}
			if (!polls[pollId]) {
				polls[pollId] = {};
			...
			polls[pollId][domId] = {
				...
				domId: domId,
				$container: $elm,
				$voteButton: $elm.find('.poll-vote-button'),
				$errors: $elm.find('.poll-errors'),
				$answers: $elm.find('.poll-answers'),
				$results: $elm.find('.poll-results')
			};
			if (queueProcessed) {
				fetchPolls(pollId);
			} else {
				queuedPolls.push(pollId);
			}
		};
